...
function dstBit(b: number) { return 1 << b; }

function dstEncodeDelta(x: number, y: number, jump: boolean): [number, number, number] {
  let b0 = 0, b1 = 0, b2 = 0;
  b2 |= dstBit(0) | dstBit(1);
  if (jump) b2 |= dstBit(7);
  if (x > 40) { b2 |= dstBit(2); x -= 81; }
  if (x < -40) { b2 |= dstBit(3); x += 81; }
  if (y > 40) { b2 |= dstBit(5); y -= 81; }
  if (y < -40) { b2 |= dstBit(4); y += 81; }
  if (x > 13) { b1 |= dstBit(2); x -= 27; }
  if (x < -13) { b1 |= dstBit(3); x += 27; }
  if (y > 13) { b1 |= dstBit(5); y -= 27; }
  if (y < -13) { b1 |= dstBit(4); y += 27; }
  if (x > 4) { b0 |= dstBit(2); x -= 9; }
  if (x < -4) { b0 |= dstBit(3); x += 9; }
  if (y > 4) { b0 |= dstBit(5); y -= 9; }
  if (y < -4) { b0 |= dstBit(4); y += 9; }
  if (x > 1) { b1 |= dstBit(0); x -= 3; }
  if (x < -1) { b1 |= dstBit(1); x += 3; }
  if (y > 1) { b1 |= dstBit(7); y -= 3; }
  if (y < -1) { b1 |= dstBit(6); y += 3; }
  if (x > 0) { b0 |= dstBit(0); x -= 1; }
  if (x < 0) { b0 |= dstBit(1); x += 1; }
  if (y > 0) { b0 |= dstBit(7); y -= 1; }
  if (y < 0) { b0 |= dstBit(6); y += 1; }
...
export function toDST(events: StitchEvent[], label?: string): Uint8Array {
  ...
  for (const e of events) {
    ...
    if (!started) {
      started = true;
      if (e.t === 'jump') { emitMove(tx, ty, true); continue; }
    }
  ...
  let name = (label || 'NEEDLESCRIPT').toUpperCase().replace(/[^A-Z0-9_]/g, '_').slice(0, 16);

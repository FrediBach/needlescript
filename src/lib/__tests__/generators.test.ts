...
const square = (s: number): Pt[] => [[-s, -s], [s, -s], [s, s], [-s, s]];
...
describe('scatter (§4.5)', () => {
  it('golden: seed 4, scatter(15) — pinned forever', () => {
    expect(run('seed 4 let p = scatter(15) print len(p) print p[0]').printed)
      .toEqual(['26', '[-12.483, 30.165]']);
  });
  ...
  it('mindist must be positive; the point cap is an error', () => {
    ...
    expect(() => scatter(0.3, disc, makeRNG(1), 20000)).toThrow(/over 20,000 points — raise mindist/);
  });
});
...
describe('voronoi (§4.5)', () => {
  ...
  it('cells tile a polygon region exactly', () => {
    ...
    const pts: Pt[] = [[-5, -5], [6, 2], [-2, 8], [9, -7], [0, 0]];
  ...
  it('small inputs: 1 point owns the region; 2 points split it', () => {
    ...
    const cells = voronoiCells([[-5, 0], [5, 0]], { kind: 'poly', pts: region });
  ...
  it('collinear points still produce cells (no triangles to lean on)', () => {
    const cells = voronoiCells([[-10, 0], [0, 0], [10, 0]], { kind: 'poly', pts: square(15) });
...
describe('triangulate & hull (§4.5)', () => {
  ...
  it('the delaunay input cap is enforced', () => {
    expect(() => triangulate(Array.from({ length: 3 }, (_, i) => [i, i * i] as Pt)))
      .not.toThrow();
    expect(() => run('let t = triangulate(filled(10001, [0, 0]))'))
...
describe('relax (§4.5, open question 4 — included)', () => {
  ...
  it('evens out spacing: nearest-neighbour distances get more uniform', () => {
    ...
    const pts: Pt[] = [];
    while (pts.length < 60) {
      const x = (rng() * 2 - 1) * 40, y = (rng() * 2 - 1) * 40;
  ...
  it('iteration count clamps 0–50 with a warning', () => {
    ...
    expect(r.warnings.some(w => w.includes('relax iterations') && w.includes('clamped to 50'))).toBe(true);
  });
});

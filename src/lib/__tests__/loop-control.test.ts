...
describe('break and continue in each loop form', () => {
  it('repeat — break ends the loop, repcount advances across continue', () => {
    expect(printed('repeat 5 [ if repcount = 3 [ break ] print repcount ]'))
      .toEqual(['1', '2']);
    expect(printed('repeat 5 [ if repcount % 2 = 0 [ continue ] print repcount ]'))
      .toEqual(['1', '3', '5']);
  });

  it('while — break stops, continue re-evaluates the condition', () => {
    expect(printed('make "i 0 while :i < 10 [ make "i :i + 1 if :i = 4 [ break ] ] print :i'))
      .toEqual(['4']);
    expect(printed('make "i 0 while :i < 6 [ make "i :i + 1 if :i % 2 = 0 [ continue ] print :i ]'))
      .toEqual(['1', '3', '5']);
  });

  it('keyword for — break, and continue applies the step', () => {
    expect(printed('for i = 1 to 10 [ if i = 4 [ break ] print i ]'))
      .toEqual(['1', '2', '3']);
    expect(printed('for i = 1 to 5 [ if i % 2 == 0 [ continue ] print i ]'))
      .toEqual(['1', '3', '5']);
  });

  it('keyword for — continue applies a negative step too', () => {
    expect(printed('for i = 5 to 1 step -2 [ if i = 3 [ continue ] print i ]'))
      .toEqual(['5', '1']);
    expect(printed('for i = 10 to 1 step -3 [ if i < 5 [ break ] print i ]'))
      .toEqual(['10', '7']);
  });

  it('classic for — both spellings behave identically', () => {
    expect(printed('for "i 1 5 1 [ if :i = 3 [ break ] print :i ]'))
      .toEqual(['1', '2']);
    expect(printed('for "i 1 5 1 [ if :i = 3 [ continue ] print :i ]'))
      .toEqual(['1', '2', '4', '5']);
  });

  it('for … in — break and continue over list elements', () => {
    expect(printed('for x in [1, 2, 3, 4] [ if x = 3 [ break ] print x ]'))
      .toEqual(['1', '2']);
    expect(printed('for x in [1, 2, 3, 4] [ if x = 2 [ continue ] print x ]'))
      .toEqual(['1', '3', '4']);
  });
...
describe('break is innermost-only', () => {
  it('break in the inner loop leaves the outer loop running', () => {
    expect(printed('repeat 3 [ repeat 5 [ if repcount = 2 [ break ] print repcount ] ]'))
      .toEqual(['1', '1', '1']);
  });

  it('outer repcount is correct again after an inner break', () => {
    expect(printed('repeat 3 [ repeat 5 [ break ] print repcount ]'))
      .toEqual(['1', '2', '3']);
  });

  it('continue in the inner loop does not skip outer iterations', () => {
    expect(printed('repeat 2 [ repeat 3 [ continue ] print repcount ]'))
      .toEqual(['1', '2']);
  });
});
...
describe('break/continue through nested blocks', () => {
  it('works through nested if blocks', () => {
    expect(printed('repeat 5 [ if repcount > 2 [ if 1 [ break ] ] print repcount ]'))
      .toEqual(['1', '2']);
  });

  it('works in else and else-if branches', () => {
    expect(printed('repeat 5 [ if repcount = 1 [ print 1 ] else if repcount = 3 [ break ] else [ print repcount ] ]'))
      .toEqual(['1', '2']);
    expect(printed('repeat 4 [ if repcount < 3 [ continue ] else [ print repcount ] ]'))
      .toEqual(['3', '4']);
  });
  ...
  it('break in a loop wrapping a complete fill leaves the fill intact', () => {
    const broken = run('repeat 3 [ up setxy -10 -10 down beginfill repeat 4 [ fd 12 rt 90 ] endfill if repcount = 1 [ break ] ]');
...
describe('break/continue parse errors', () => {
  ...
  it('a REPL-appended bare break errors; an appended loop with break works', () => {
    ...
    expect(printed('fd 1\nrepeat 9 [ if repcount > 2 [ break ] print repcount ]'))
      .toEqual(['1', '2']);
  });

  it('break is lexical: a procedure called from a loop is not "inside" it', () => {
    expect(() => run('def helper() [ break ] repeat 3 [ helper() ]'))
      .toThrow(/the loop is in the caller/);
    expect(() => run('to helper break end repeat 3 [ helper ]'))
      .toThrow(/use return \(or exit\/output\) to leave the procedure/);
  });

  it('a loop inside the procedure makes them valid again', () => {
    expect(printed('def f() [ repeat 5 [ if repcount = 2 [ break ] print repcount ] ] f()'))
      .toEqual(['1']);
  });
...
describe('the while-true search idiom', () => {
  it('while true [ … break ] is the sanctioned search loop', () => {
    expect(printed('make "i 0 while true [ make "i :i + 1 if :i = 7 [ break ] ] print :i'))
      .toEqual(['7']);
  });
...
describe('break vs exit/return', () => {
  it('exit inside a loop leaves the whole procedure, not just the loop', () => {
    expect(printed('to f repeat 10 [ if repcount = 3 [ exit ] print repcount ] print 99 end f'))
      .toEqual(['1', '2']);
  });

  it('return e inside a loop in a reporter returns the value', () => {
    expect(printed('def f() [ for i = 1 to 10 [ if i == 4 [ return i * 10 ] ] return 0 ] print f()'))
      .toEqual(['40']);
  });
});
...
describe("the ' character", () => {
  it('is reserved with a forward-looking error', () => {
    expect(() => run("print 'hi'")).toThrow(/single-quote strings are reserved for a future version/);

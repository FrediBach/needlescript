...
import {
  apply, compose, mTranslate, mRotate, mRotateAbout, mMirror,
} from '../affine.ts';
...
describe('isometries map the whole event stream', () => {
  ...
  it('translate', () => {
    expect(clean(ev(`translate 7 -3 [ ${PROG} ]`)))
      .toEqual(mapped(ev(PROG), mTranslate(7, -3)));
  });

  it('rotate about the origin', () => {
    expect(clean(ev(`rotate 37 [ ${PROG} ]`)))
      .toEqual(mapped(ev(PROG), mRotate(37)));
  });

  it('rotateabout an explicit pivot', () => {
    expect(clean(ev(`rotateabout 90 5 5 [ ${PROG} ]`)))
      .toEqual(mapped(ev(PROG), mRotateAbout(90, 5, 5)));
  });

  it('mirror', () => {
    expect(clean(ev(`mirror 25 [ ${PROG} ]`)))
      .toEqual(mapped(ev(PROG), mMirror(25)));
  });

  it('a satin column maps as a rigid body under rotation', () => {
    const SAT = 'satin 2.4 down fd 14 satin 0';
    expect(clean(ev(`rotate 50 [ ${SAT} ]`)))
      .toEqual(mapped(ev(SAT), mRotate(50)));
  });
  ...
  it('a transformed fill covers the transformed boundary', () => {
    ...
    const m = mRotate(35);
    const corners: [number, number][] = [[0, 0], [0, 16], [16, 16], [16, 0]];
  ...
  it('nested isometries compose inside-out', () => {
    expect(clean(ev(`translate 10 0 [ rotate 90 [ ${PROG} ] ]`)))
      .toEqual(mapped(ev(PROG), compose(mTranslate(10, 0), mRotate(90))));
  });
});
...
describe('block transform ≡ pure x* function', () => {
  ...
  const same = (block: string, fn: string) =>
  ...
  it('skew ≡ the raw transform matrix it builds', () => {
    const tx = Math.tan(10 * Math.PI / 180), ty = Math.tan(5 * Math.PI / 180);
...
describe('stitch physics run on post-transform geometry', () => {
  it('stitch length stays physical under scale (split in hoop space)', () => {
    ...
    const scaled = ev('lock 0 scale 4 [ stitchlen 2.5 down fd 10 ]').filter(e => e.t === 'stitch');
  ...
  it('pull compensation is NOT scaled by the transform', () => {
    ...
    const evs = ev('pullcomp 0.5 satin 2 down fd 20 satin 0')
      .filter(e => e.t === 'stitch');
    const span = (es: StitchEvent[]) =>
      Math.max(...es.map(e => e.x)) - Math.min(...es.map(e => e.x));
    const scaled = ev('scale 2 [ pullcomp 0.5 satin 2 down fd 20 satin 0 ]')
      .filter(e => e.t === 'stitch');
  ...
  it('non-uniform scale makes satin width direction-dependent', () => {
    ...
    const east = ev('scalexy 2 1 [ satin 2 down rt 90 fd 12 satin 0 ]').filter(e => e.t === 'stitch');
    const xSpan = (es: StitchEvent[]) => Math.max(...es.map(e => e.x)) - Math.min(...es.map(e => e.x));
    const ySpan = (es: StitchEvent[]) => Math.max(...es.map(e => e.y)) - Math.min(...es.map(e => e.y));
...
describe('Option A: reporters and randomness are transform-invariant', () => {
  it('xcor/ycor report local (pre-transform) coordinates', () => {
    expect(printed('scale 3 [ fd 10 print xcor print ycor ]'))
      .toEqual(printed('fd 10 print xcor print ycor'));
  ...
  it('wrapping a motif in a transform does not reshuffle the RNG', () => {
    expect(printed('seed 7 print random(100) translate 50 0 [ print random(100) ]'))
      .toEqual(printed('seed 7 print random(100) print random(100)'));
  });
});
...
describe('pure path functions are pure and correct', () => {
  it('xlate returns a new path and leaves the original untouched', () => {
    expect(printed('let p = [[0,0],[1,1]] let q = xlate(p, 5, 0) print q print p'))
      .toEqual(['[[5, 0], [6, 1]]', '[[0, 0], [1, 1]]']);
  });

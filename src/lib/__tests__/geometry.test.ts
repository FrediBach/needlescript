...
describe('clippaths (§4.6)', () => {
  ...
  it('a bad op word is a parse error with did-you-mean', () => {
    expect(() => run('let u = clippaths([[0,0],[1,0],[1,1]], [[0,0],[1,0],[1,1]], "unoin")'))
  ...
  it('the op must be a quoted word, at parse time', () => {
    expect(() => run('let u = clippaths([[0,0],[1,0],[1,1]], [[0,0],[1,0],[1,1]], 3)'))
      .toThrow(/clippaths needs a quoted word here/);
    expect(() => run('let u = clippaths([[0,0],[1,0],[1,1]], [[0,0],[1,0],[1,1]])'))
      .toThrow(/clippaths\(…\) expects 3 arguments, got 2/);
  });
});
...
describe('inpath (§4.6)', () => {
  ...
  it('even-odd rule (consistent with fills): the hole of a self-overlap is out', () => {
    ...
    expect(pointInRegion([0, 0], [
      [-10, -10], [10, -10], [10, 10], [-10, 10], [-10, -10],
      [-4, -4], [4, -4], [4, 4], [-4, 4], [-4, -4],
    ])).toBe(false);
    expect(pointInRegion([7, 0], [
      [-10, -10], [10, -10], [10, 10], [-10, 10], [-10, -10],
      [-4, -4], [4, -4], [4, 4], [-4, 4], [-4, -4],
    ])).toBe(true);
  });
  ...
  it('a region needs at least 3 points', () => {
    expect(() => run('print inpath([0, 0], [[1, 1], [2, 2]])'))
      .toThrow(/inpath: expected a path of at least 3 points/);
  });
});
...
describe('geometry limits (§8)', () => {
  it('offsetpath input vertex cap', () => {
    expect(() => run('let r = offsetpath(filled(50001, [0, 0]), 1)'))
      .toThrow(/too many vertices/);
  });
});

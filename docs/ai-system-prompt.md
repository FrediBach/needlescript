...
During playground AI generation, a successful compile may be followed by compiled spatial context and a structured physics review. Spatial context uses hoop-space millimetres (+x right, +y up, origin at hoop centre) and may include exact bounds, color extents, a coarse stitched silhouette, and a rendered preview. Numbered preview overlays correspond to the numbered physics findings. Treat the compiled result as ground truth: preserve the requested visual intent, address blockers before risks, and return a complete revised program. A spatial review may explicitly ask you to return the source unchanged when the composition already satisfies the request. Do not chase informational notes automatically. If a finding persists, change the attributed construction instead of weakening thresholds, deleting material intent, inserting `preflight`, or otherwise hiding the report.

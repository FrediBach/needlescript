...
You can also bring artwork _in_: **Import SVG** (a button, or drag and drop) converts SVG structure into _editable_ NeedleScript code. Quick import and **Import with options** use the same conversion pipeline. Fill and stroke become separate operations over one shared named geometry, compound paths honor SVG `evenodd` and `nonzero` winding, and nested groups, inherited paint, transforms, physical stroke widths, and the selected hoop field are retained. It supports `<path>` (M L H V C S Q T A Z), rect/circle/ellipse/line/polyline/polygon. Opaque `<linearGradient>` fills with 2–8 stops become density-neutral multithread row blends: the SVG vector, stop offsets, inheritance, coordinate units, and transforms stay intact, while the options inspector exposes aggregate row pitch and stitch length. Thread mapping applies to every stop. Radial, repeating, transparent, and stroke gradients remain visible findings because linear rows would not preserve their meaning. Other unknown paints and unsupported elements are likewise shown as findings rather than changed to a guessed color. In the options workspace, curve paths can remain editable `// [curve]` specs flattened at runtime with `curveflat` — a useful handoff from a logo into generative code.
